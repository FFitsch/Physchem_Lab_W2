...
# ------------------------ CONFIG --------------------------
volume_file = './180-Grad/1_S.csv'
pressure_file = './180-Grad/2_S.csv'
horiz_cutoff_left = -5
...
# import data
v_data_raw = pd.read_csv(volume_file, skiprows=28, usecols=[v_col_time,v_col_volt], names=['time','value'])
p_data_raw = pd.read_csv(pressure_file, skiprows=28, usecols=[p_col_time,p_col_volt], names=['time','value'])
...
p_data = p_data_raw[v_data_raw['time'] >= horiz_cutoff_left]
p_data = p_data_raw[v_data_raw['time'] <= horiz_cutoff_right]

# vertical scale corrections
v_data.loc[:, 'value'] += v_vert_pos
v_data.loc[:, 'value'] *= v_vert_scale

p_data.loc[:, 'value'] += p_vert_pos
p_data.loc[:, 'value'] *= p_vert_scale
...
v_data.loc[:, 'value'] = (v_data.value + 41)/56
p_data.loc[:, 'value'] = ((p_data.value - 1.05)/3.1) + ambient_pressure


#lets find some local extreme values
v_data.loc[:, 'max'] = v_data.iloc[argrelextrema(v_data.value.values, np.greater_equal, order = 100)[0]]['value']
p_data.loc[:, 'max'] = p_data.iloc[argrelextrema(p_data.value.values, np.greater_equal, order = 100)[0]]['value']
